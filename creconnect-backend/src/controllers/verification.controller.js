...
const getStatus  = async (req, res, next) => {
  try { ok(res, { verifications: await svc.getStatus(req.user.id) }); } catch (e) { next(e); }
};
...
const uploadDoc = async (req, res, next) => {
  try {
    ...
    ok(res, { documentId: result.public_id, secureUrl: result.secure_url }, 'Document uploaded');
  } catch (e) { next(e); }
};
